...
```{r tidy=TRUE}
packages = c('stringr', 'tidyr','tidyverse', 'dplyr', 'tm', 'lubridate', 
...
read in employee records to form nodes
...
employee_email_agg <- employee_email %>%
  ...
  select(From, To, Day, SentTime, Type, Subject) %>%
...
email_eachday <- employee_email_agg %>%
  ...
  group_by(Source, Day, Type, Subject) %>%
  summarise(n=n()) %>%
  filter(n==53) %>%
...
```{r}
employee_edges_2 <- email_eachday %>%
  mutate(delete="yes") %>%
...
```
```{r}
email_matrix2 <- employee_edges_2 %>%
  filter(Type=="direct") %>%
  select(Source, Target) %>%
  group_by(Source, Target) %>%
  summarise(n=n()) 
